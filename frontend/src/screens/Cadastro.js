...
export default class Cadastro extends Component {
    ...
    submitPessoa = async () => {
        if (!this.state.pessoaNome || !this.state.pessoaEmail || !this.state.pessoaTelefone) {
        ...
        if (this.state.pessoaId) {
            try {
                await axios.put(`${baseApiUrl}/pessoas/${this.state.pessoaId}`, {
                    nome: this.state.pessoaNome,
                    email: this.state.pessoaEmail,
                    telefone: this.state.pessoaTelefone,
                })
              } catch(err) {
                alert(err)
              }
    ...
    render() {
        ...
        <Link className="mt-5" to="/"><Button variant="secondary">Ir para agenda</Button></Link> 
        </div>
        )
}
}